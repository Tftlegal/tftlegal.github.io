...
summary: "**Wazuh** — это комплексная платформа безопасности с открытым исходным кодом, объединяющая возможности **SIEM** (Security Information and Event Management) и **XDR** (Extended Detection and Response)."
...
Данное руководство описывает процесс развертывания **Wazuh** — комплексной платформы безопасности (SIEM/XDR). 

*Примечание: На текущий момент актуальной стабильной версией является Ubuntu 24.04 LTS. Инструкции полностью применимы к архитектуре последующих LTS-выпусков.*
...
Wazuh крайне чувствителен к ресурсам из-за работы движка индексации.

### Системные требования (Минимум для All-in-one):
*   **CPU:** 4 ядра.
*   **RAM:** 8 ГБ (рекомендуется 16 ГБ для стабильной работы Indexer).
*   **Disk:** 50 ГБ+ (SSD крайне предпочтителен для скорости индексации).
...
1. **Обновление системы:**
   ...
   ```
2. **Настройка виртуальной памяти (Критично для Indexer):**
   ```bash
   sudo sysctl -w vm.max_map_count=262144
   echo "vm.max_map_count=262144" | sudo tee -a /etc/sysctl.conf
   ```
3. **Создание Swap-файла (если RAM < 16ГБ):**
   ```bash
...
Используем современный метод добавления ключей для обеспечения безопасности.

```bash
# Установка необходимых утилит
sudo apt install -y curl wget apt-transport-https gnupg2 ca-certificates libcap2-bin
...
echo "deb [signed-by=/usr/share/keyrings/wazuh.gpg] https://packages.wazuh.com/4.x/apt/ any main" | sudo tee /etc/apt/sources.list.d/wazuh.list
sudo apt update
```

### Развертывание (Метод All-in-one):
Для быстрой настройки сервера «всё в одном» используйте официальный скрипт автоматизации. Он автоматически сконфигурирует сертификаты TLS, Indexer, Manager и Dashboard.

```bash
curl -sO https://packages.wazuh.com/install_central_node.sh
sudo bash install_central_node.sh
```
*После завершения скрипт выведет IP-адрес, логин (`admin`) и временный пароль.*
...
### Шаг 0: Проверка связи c агентами
В Dashboard в разделе **Agents** статус должен измениться на **Active** в течение 1-2 минут после запуска агента.

### Шаг 1: Проверка статусов
Проверьте, что все службы активны:
```bash
sudo systemctl status wazuh-indexer wazuh-manager wazuh-dashboard
```

### Шаг 2: Тест безопасности (Brute Force)
Имитируйте атаку на SSH, чтобы проверить срабатывание правил:
```bash
# Выполните на клиентском сервере
for i in {1..10}; do ssh fakeuser@localhost; done
```

### Шаг 3: Проверка FIM (File Integrity Monitoring)
1. В Dashboard найдите путь, который вы хотите мониторить (например, `/etc/hosts`).
2. Измените файл на клиенте: `echo "127.0.0.1 test" | sudo tee -a /etc/hosts`.
3. Проверьте раздел **Integrity Monitoring** в Dashboard — изменение должно появиться в течение минуты.

### Итоговый чек-лист:
- [ ] **Indexer:** Порт 9200 доступен локально.
- [ ] **Manager:** Принимает соединения на 1514.
- [ ] **Dashboard:** Доступен по HTTPS (при первом входе нажмите «Advanced» -> «Proceed to site»).
- [ ] **Agents:** Все клиенты имеют статус **Active**.
- [ ] **Alerts:** Тестовый алерт Brute Force отобразился в журнале.
