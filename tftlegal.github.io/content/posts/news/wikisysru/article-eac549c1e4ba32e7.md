...
summary: "В Kubernetes развертывание приложений обеспечивают контроллеры Deployment и ReplicaSet. Они управляются компонентом kube-controller-manager. На самом нижнем уровне находятся pods, объединяющие контейнеры приложения. ReplicaSet поддерживает заданное количество идентичных pods. Deployment работает над ReplicaSet и обеспечивает стабильные обновления и откаты версий. Вместе эти контроллеры помогают запускать и поддерживать работу приложения в кластере."
---
...
В Kubernetes развертывание приложений обеспечивают контроллеры Deployment и ReplicaSet.
Они управляются компонентом kube-controller-manager.
На самом нижнем уровне находятся pods, объединяющие контейнеры приложения.
ReplicaSet поддерживает заданное количество идентичных pods.
Deployment работает над ReplicaSet и обеспечивает стабильные обновления и откаты версий.
Вместе эти контроллеры помогают запускать и поддерживать работу приложения в кластере.
