...
summary: "ConfigMap позволяет отделить конфигурацию приложения от его кода и образа. Это устраняет привязку настроек к конкретному окружению, так как параметры можно задать отдельно для каждого окружения. Благодаря этому один и тот же образ можно использовать в dev, test"
---
...
ConfigMap позволяет отделить конфигурацию приложения от его кода и образа. Это устраняет привязку настроек к конкретному окружению, так как параметры можно задать отдельно для каждого окружения. Благодаря этому один и тот же образ можно использовать в dev, test
