...
source: "godev"
original_url: "https://go.dev/blog/generic-methods"
summary: "В версии Go 1.27 в язык добавлены generic methods — методы с параметрами типов. Это долгожданная языковая возможность, которая расширяет возможности Go. Теперь методы могут работать с разными типами данных без потери типобезопасности. Такая функция упрощает написание переиспользуемого и более выразительного кода. Обновление делает Go ближе к языкам с полноценной поддержкой дженериков."
---
...
В версии Go 1.27 в язык добавлены generic methods — методы с параметрами типов.
Это долгожданная языковая возможность, которая расширяет возможности Go.
Теперь методы могут работать с разными типами данных без потери типобезопасности.
Такая функция упрощает написание переиспользуемого и более выразительного кода.
Обновление делает Go ближе к языкам с полноценной поддержкой дженериков.
...
The generics[proposal](https://go.dev/issue/43651)reasoned that because generic interface methods are difficult to implement efficiently (discussed[later](#the-trouble-with-generic-interface-methods)), it didn’t make sense to add type parameters to non-interface (or “concrete”) methods either. In that view, methods are primarily the means of implementing an interface.
...
For`T.M`to participate in interface implementation, there would need to exist a suitable generic method`I.M`:
