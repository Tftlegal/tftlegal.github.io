...
source: "godev"
original_url: "https://go.dev/blog/inliner"
summary: "Текст объясняет, как новый в Go 1.26 исходный инлайнер работает на уровне исходного кода, а не только на уровне сгенерированного байткода.   Он позволяет компилятору раскрывать вызовы функций прямо в исходных выражениях, сохраняя более понятную структуру кода и метаданные.   Это упрощает оптимизации и даёт больше возможностей для статического преобразования вызовов.   Для self-service API-миграций такой механизм может автоматически заменять устаревшие вызовы новыми реализациями в клиентском коде.   Разработчики смогут переходить на новые API без массового ручного изменения кода, сохраняя совместимость и контроль над миграцией.   Таким образом, инлайнер становится инструментом для постепенной, безопасной и автоматизированной модернизации публичных интерфейсов."
---
...
Текст объясняет, как новый в Go 1.26 исходный инлайнер работает на уровне исходного кода, а не только на уровне сгенерированного байткода.  
Он позволяет компилятору раскрывать вызовы функций прямо в исходных выражениях, сохраняя более понятную структуру кода и метаданные.  
Это упрощает оптимизации и даёт больше возможностей для статического преобразования вызовов.  
Для self-service API-миграций такой механизм может автоматически заменять устаревшие вызовы новыми реализациями в клиентском коде.  
Разработчики смогут переходить на новые API без массового ручного изменения кода, сохраняя совместимость и контроль над миграцией.  
Таким образом, инлайнер становится инструментом для постепенной, безопасной и автоматизированной модернизации публичных интерфейсов.

## Полная статья

Go 1.26 contains an all-new implementation of the`go fix`subcommand, designed to help you keep your Go code up-to-date and modern. For an introduction, start by reading our[recent post](https://go.dev/blog/gofix)on the topic. In this post, we’ll look at one particular feature, the source-level inliner.

While`go fix`has several bespoke modernizers for specific new language and library features, the source-level inliner is the first fruit of our efforts to provide “[self-service](https://go.dev/blog/gofix#self-service)” modernizers and analyzers. It enables any package author to express simple API migrations and updates in a straightforward and safe way. We’ll first explain what the source-level inliner is and how you can use it, then we’ll dive into some aspects of the problem and the technology behind it.
...
In Go 1.16, the`ioutil.ReadFile`function, which reads the content of a file, was deprecated in favor of the new`os.ReadFile`function. In effect, the function was renamed, though of course Go’s[compatibility promise](https://go.dev/doc/go1compat)prevents us from ever removing the old name.
