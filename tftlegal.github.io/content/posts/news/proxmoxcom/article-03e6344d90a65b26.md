---
title: "Proxmox VE 4.0 with Linux Containers (LXC) and new HA Manager Released"
date: 2015-09-25T15:38:04Z
source: "proxmoxcom"
original_url: "https://www.proxmox.com/en/about/company-details/press-releases/proxmox-ve-4-0-released"
summary: "6 октября 2015 года Proxmox Server Solutions выпустила Proxmox Virtual Environment 4.0. Это решение с открытым исходным кодом позволяет управлять KVM-виртуальными машинами и контейнерами на одном хосте через веб-интерфейс. Новая версия построена на Debian Jessie и включает обновление ядра, QEMU и пакетов Ceph. Главные нововведения — Proxmox VE HA Manager для высокодоступных кластеров и встроенный HA Simulator для тестирования отказов. Вместо OpenVZ теперь используется Linux Containers (LXC), которые интегрированы в модель хранения и управляются через веб-интерфейс или PCT. Также добавлены встроенная консоль NoVNC, DRBD9 как технологический предпросмотр и полная поддержка IPv6 для виртуальных машин, контейнеров и хостов. Proxmox VE 4.0 доступна для скачивания, а версия 3.x будет поддерживаться примерно до конца обновлений Debian Wheezy в апреле 2016 года."
---

# Proxmox VE 4.0 with Linux Containers (LXC) and new HA Manager Released

## Краткое содержание

6 октября 2015 года Proxmox Server Solutions выпустила Proxmox Virtual Environment 4.0.
Это решение с открытым исходным кодом позволяет управлять KVM-виртуальными машинами и контейнерами на одном хосте через веб-интерфейс.
Новая версия построена на Debian Jessie и включает обновление ядра, QEMU и пакетов Ceph.
Главные нововведения — Proxmox VE HA Manager для высокодоступных кластеров и встроенный HA Simulator для тестирования отказов.
Вместо OpenVZ теперь используется Linux Containers (LXC), которые интегрированы в модель хранения и управляются через веб-интерфейс или PCT.
Также добавлены встроенная консоль NoVNC, DRBD9 как технологический предпросмотр и полная поддержка IPv6 для виртуальных машин, контейнеров и хостов.
Proxmox VE 4.0 доступна для скачивания, а версия 3.x будет поддерживаться примерно до конца обновлений Debian Wheezy в апреле 2016 года.
...
###### Highly available Clusters with Proxmox VE HA Manager
...
> The move from OpenVZ to LXC allowed us to finally update to the latest and greatest kernel improving even more the security, scalability and robustness of the whole Proxmox VE platform. We are also really excited that with the new Proxmox HA manager our users can now setup and deploy highly available clusters easily and simply configure them in the GUI.Martin Maurer - CEO of Proxmox Server Solutions GmbH.

**Important Wiki Articles:**
[https://pve.proxmox.com/wiki/Linux_Container](https://pve.proxmox.com/wiki/Linux_Container)
[https://pve.proxmox.com/wiki/Convert_OpenVZ_to_LXC](https://pve.proxmox.com/wiki/Convert_OpenVZ_to_LXC)
...
https://www.proxmox.com/en/about/company-details/press-releases/proxmox-ve-4-0-released
